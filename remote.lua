...
--
-- Update History: v1.1 Intergrated Compass - v1.2 Intergrated Asynchronous Connection - v1.3 Lion / Windows Fix
--
...
local connectionKey = "CORONAREMOTE1.3"
local connectionResponseKey = "CORONAREMOTE1.3CONNECTED"
local errorCount = 0
...
local function asynchronousConnection( event )
	...
	if event.isError then
		...
		if tcpClientMessage[ 1 ] == "CR" then
				
			-- Reset Timeout
			errorCount = 0
				
			-- Update Values
			xGravity = tonumber( tcpClientMessage[ 2 ] )
			yGravity = tonumber( tcpClientMessage[ 3 ] )
			zGravity = tonumber( tcpClientMessage[ 4 ] )
			xInstant = tonumber( tcpClientMessage[ 5 ] )
			yInstant = tonumber( tcpClientMessage[ 6 ] )
			zInstant = tonumber( tcpClientMessage[ 7 ] )
			isShake = tcpClientMessage[ 8 ]
			if isShake == "1" then
				isShake = true
			else
				isShake = false
			...
			if compass == true then
				magnetic = tonumber( tcpClientMessage[ 9 ] )
			end
